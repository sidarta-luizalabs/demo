node {
    ...
    stage " 1 | Build w/ Docker plugin"
        ...
        docker.withRegistry('https://index.docker.io/v1/', 'dockerhub') {
            def img = docker.build("sidartaoliveira/demo:${id}")
            img.push()
        }
    
    stage " 2 | Testing image"
        sh "echo docker run -d -p 8090:8080 --name demo sidartaoliveira/demo:${id}"
        sh "echo curl -v http://localhost:8090/"
        sh "echo docker rm demo --force"
    
    stage " 3 | Deployment"
        sh "kubectl get deployment demo -o yaml | sed 's%sidartaoliveira/demo.*%sidartaoliveira/demo:${id}%' > /tmp/demo.yaml"
